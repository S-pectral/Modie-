...
// @version      3.8
// @description  V3.8 Hybrid Aimbot (Sticky + Precision)
// @author       Modie
// @match        *://krunker.io/*
...
(function () {
    'use strict';
    console.log("MODIE: Script Initializing...");

    if (!window.THREE) {
        console.error("MODIE: THREE.js not loaded! @require failed or missing.");
        alert("MODIE Error: THREE.js missing. Check extension/internet.");
        return;
    }
...
`;
    ...
    let activeTab = 'Main';
    const settingsMap = {
        'Main': ['aimbotEnabled', 'aimbotOnRightMouse', 'humanize', 'aimOffsetY'],
        'Visuals': ['espEnabled', 'espLines', 'espLineColor', 'chams', 'chamsRainbow', 'wireframe', 'colEnemy', 'colTeam', 'fogEnabled', 'fogColor', 'fogDistance'],
        'Config': ['smoothSpeed', 'fovCircle']
    };
    ...
    `;
        ...
        const tabVisuals = gui.querySelector('#tab-Visuals');
        const tabConfig = gui.querySelector('#tab-Config');

        function switchTab(tab) {
            activeTab = tab;
            tabMain.className = `modie-tab ${tab === 'Main' ? 'active-tab' : ''}`;
            tabVisuals.className = `modie-tab ${tab === 'Visuals' ? 'active-tab' : ''}`;
        ...
        tabVisuals.onclick = () => switchTab('Visuals');
    ...
    function renderItems(container) {
        ...
        for (const key of keys) {
            ...
            // Generic Slider Logic (Speed, Offsets, Fog Dist)
            else if (typeof settings[key] === 'number') {
                const isFloat = key === 'smoothSpeed';
                const step = isFloat ? 0.1 : (key === 'fogDistance' ? 10 : 0.5);
                const min = key === 'fogDistance' ? 0 : 0;
                // AimOffset = 20, Fog = 1000, SmoothSpeed = 1.0
                const max = key === 'fogDistance' ? 1000 : (key === 'aimOffsetY' ? 20 : (key === 'smoothSpeed' ? 1.0 : 2));

                item.innerHTML = `
                <span class="modie-key"></span>
                <span style="flex-grow:1">${formatName(key)}</span>
                <input type="range" class="modie-slider" min="${min}" max="${max}" step="${step}" value="${settings[key]}" style="width:60px;">
                <span class="modie-val on" style="width:30px; text-align:right;">${settings[key]}</span>
            `;

                const slider = item.querySelector('input');
                const valDisplay = item.querySelector('.modie-val');
                ...
                item.onclick = (e) => { if (e.target !== slider) slider.focus(); };
            }
    ...
    function updatePlayerCount(count) {
        ...
        if (el) {
            if (!scene) el.innerHTML = `MODIE V3.8 <span style="color:#ff5555;">(Injecting...)</span>`;
            else el.innerHTML = `MODIE V3.8 <span style="color:#fff; font-size:10px;">(Players: ${count})</span>`;
        }
    }
    ...
    function animate() {
        ...
        if (settings.fogEnabled) {
            if (!scene.fog || scene.fog.color.getHexString() !== settings.fogColor.replace('#', '') || scene.fog.far !== settings.fogDistance) {
                scene.fog = new THREE.Fog(settings.fogColor, 0, settings.fogDistance);
            }
        } else {
        ...
        for (const player of players) {
            ...
            player.traverse((child) => {
                if (child.isMesh && child.material) {
                    if (settings.chams || settings.wireframe) {
                        ...
                        if (isTeammate) {
                            child.material.emissive?.setHex(parseInt(settings.colTeam.replace('#', '0x')));
                            child.material.color?.setHex(parseInt(settings.colTeam.replace('#', '0x')));
                        } else {
                            if (settings.chamsRainbow) {
                                child.material.emissive?.setHSL(rainbowHue, 1, 0.6);
                                child.material.color?.setHSL(rainbowHue, 1, 0.6);
                            } else {
                                child.material.emissive?.setHex(parseInt(settings.colEnemy.replace('#', '0x')));
                                child.material.color?.setHex(parseInt(settings.colEnemy.replace('#', '0x')));
                            }
                        }
            ...
            if (espData.group) {
                ...
                if (player === myPlayer || (player.position.x === myPlayer.position.x && player.position.z === myPlayer.position.z)) {
                    ...
                    if (espData.line) espData.line.visible = false;
                } else {
                    ...
                    if (settings.espLines) {
                        if (!espData.line) {
                            ...
                            espData.line = new THREE.LineSegments(geom, mat);
                            espData.line.frustumCulled = false;
                            myPlayer.add(espData.line); // Attach to self for local space coords
                        }

                        // Calculate target pos in local space of myPlayer
                        const v = player.position.clone();
                        v.y += 5; // Aim at center/head area
                        v.applyMatrix4(new THREE.Matrix4().copy(myPlayer.matrix).invert());

                        const arr = espData.line.geometry.attributes.position.array;
                        // Index 0-2 is origin [0,0,-5] (Crosshair)
                        // Index 3-5 is target
                        arr[3] = v.x; arr[4] = v.y; arr[5] = v.z;
                        espData.line.geometry.attributes.position.needsUpdate = true;

                        espData.line.material.color.set(settings.espLineColor);
                        espData.line.visible = true;

                    } else if (espData.line) {
        ...
        for (const [uuid, data] of espMap) {
            if (!activePlayerSet.has(uuid)) {
                ...
                if (data.line && myPlayer) myPlayer.remove(data.line); // Remove tracer from self
                espMap.delete(uuid);
            }
        }

        // Sticky Target Logic (from b.js)
        let lockedTarget = null;

        // Validate current locked target
        if (lockedTarget) {
            const stillValid = players.includes(lockedTarget) &&
                lockedTarget.visible &&
                (lockedTarget.health === undefined || lockedTarget.health > 0);
            if (!stillValid) lockedTarget = null;
        }

        // Find new target if unlocked
        if (!lockedTarget) {
            let minWorldDist = Infinity;
            for (const player of players) {
                const isTeammate = (myTeam != null && player.team != null && player.team == myTeam);
                if (isTeammate) continue;
                if (settings.aimbotWallCheck && !player.visible) continue;
                    ...
                    if (screenPos.z < 1) {
                        ...
                        if (distToCrosshair <= 150) {
                            ...
                            if (dist < minWorldDist) {
                                minWorldDist = dist;
                                lockedTarget = player; // Lock onto new closest
                            }
                        }
                    }
                }
            ...
            if (shouldAim) {
                ...
                if (cam) {
                    if (targetHead) {
                        ...
                        tempVector.y += 2.0; // a.js hardcoded head offset
                    } else {
                        tempVector.copy(targetPlayer.position);
                        tempVector.y += 11; // a.js hardcoded body offset
                    }
                    ...
                    const speed = settings.smoothSpeed || 0.2; // a.js speed logic (ignores humanize)
    ...
    animate();
    console.log("Modie V3.8 Loaded");
    console.log("Credit: Modie V3.8");
})();
